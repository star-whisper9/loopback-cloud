  ...
  it("falls back to best Accept-Language match", () => {
    expect(
      resolveLocale({
        url: new URL(base),
        cookie: undefined,
        acceptLanguages: ["en-US", "en", "zh-CN"],
      }),
    ).toBe("en");
    expect(
      resolveLocale({
...
describe("t()", () => {
  ...
  it("throws on missing key (Fail-Fast)", () => {
    const t = createT(zh);
    expect(() => t("nonexistent.deeply.nested" as any)).toThrow(
      /i18n missing key/,
    );
  });
});
