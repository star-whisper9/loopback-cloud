...
import { MAX_FIREWALL_RULES, type FirewallRule, type FirewallProtocol, type FirewallRuleAction } from "~/lib/machineTypes";
...
function isValidSource(s: string): boolean {
  ...
  if (Number(o1) > 255 || Number(o2) > 255 || Number(o3) > 255 || Number(o4) > 255) return false;
...
export function Firewall() {
  ...
  function handleAdd() {
    ...
    if (!isValidSource(source)) errs.source = t("console.firewall.invalidSource");
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-[var(--color-fg-muted)]/60">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {atLimit ? t("console.firewall.ruleLimitReached") : t("console.firewall.addRule")}
        </Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-white/10">
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-white/10 text-xs uppercase tracking-wider text-[var(--color-fg-muted)]">
          ...
          <tbody>
            {rules.map((r) => (
              ...
              >
                <td className="px-4 py-3 font-mono text-[var(--color-fg)]">{r.name}</td>
                <td className="px-4 py-3 font-mono text-[var(--color-fg-muted)]">{r.source}</td>
                <td className="px-4 py-3 uppercase text-[var(--color-fg-muted)]">{r.protocol}</td>
      ...
      <AnimatePresence>
        {undoRule && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-lg border border-white/10 bg-[var(--color-surface)] px-4 py-3 text-sm shadow-xl"
          >
      ...
      <Modal>
        <Modal.Backdrop isOpen={addOpen} onOpenChange={(v) => !v && setAddOpen(false)}>
          <Modal.Container>
            <Modal.Dialog className="sm:max-w-md">
              <Modal.CloseTrigger />
              <Modal.Header>
                <Modal.Heading>{t("console.firewall.addModalTitle")}</Modal.Heading>
              </Modal.Header>
              <Modal.Body className="space-y-4 p-6">
                <div>
                  ...
                  {addErrors.name && (
                    <p className="mt-1 text-xs text-red-400">{addErrors.name}</p>
                  )}
                </div>
                <div>
                  <Label className="mb-1 block text-sm text-[var(--color-fg)]">
                  ...
                  <Input value={source} onChange={(e) => setSource((e.target as HTMLInputElement).value)} />
                  {addErrors.source && (
                    <p className="mt-1 text-xs text-red-400">{addErrors.source}</p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select
                      ...
                      onSelectionChange={(k) => setProtocol(k as FirewallProtocol)}
                    >
                      <Select.Trigger>
                        <Select.Value />
                        <Select.Indicator />
                      ...
                      <Select.Popover>
                        <ListBox>
                          {PROTOCOLS.map((p) => (
                            <ListBox.Item key={p} id={p} textValue={p.toUpperCase()}>
                              {p.toUpperCase()}
                  ...
                  <div>
                    ...
                    <Input value={port} onChange={(e) => setPort((e.target as HTMLInputElement).value)} />
                    {addErrors.port && (
                      <p className="mt-1 text-xs text-red-400">{addErrors.port}</p>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <Select
                    selectedKey={action}
                    onSelectionChange={(k) => setAction(k as FirewallRuleAction)}
                  >
                    <Select.Trigger>
                      <Select.Value />
                      <Select.Indicator />
